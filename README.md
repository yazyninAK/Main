...
Список отсортирован сайтом по дате публикации/поднятия (новые сверху),
поэтому скрипт подгружает страницы по 32 объявления одну за другой, пока не
встретит объявление, которое уже видел раньше (до 10 страниц за один запуск,
для защиты от зависания при сбоях). При проверке дважды в день этого более
чем достаточно, чтобы не пропустить ни одного нового объявления.
